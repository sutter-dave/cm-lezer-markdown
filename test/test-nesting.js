...
function findMounts(tree) {
    var _a;
    let result = [];
    for (let cur = tree.cursor(); cur.next();) {
        let mount = (_a = cur.tree) === null || _a === void 0 ? void 0 : _a.prop(NodeProp.mounted);
        if (mount)
            result.push({ at: cur.from, mount });
    }
    return result;
}
function test(doc, ...nest) {
    return () => {
...
<div id=x>
  Hello &amp; goodbye
    ...
    it("parses inline HTML", test(`Paragraph with <em>inline tags</em> in it.`, ["Element(OpenTag(StartTag,TagName,EndTag))", 15, 19], ["CloseTag(StartCloseTag,TagName,EndTag)", 30, 35]));
...
~~~`, ["EntityReference", 9, 16]));
    it("can parse disjoint ranges", () => {
        let tree = parser.parse(`==foo\n==\n==ba==r\n==`, undefined, [{ from: 2, to: 6 }, { from: 8, to: 9 }, { from: 11, to: 13 }, { from: 15, to: 17 }]);
