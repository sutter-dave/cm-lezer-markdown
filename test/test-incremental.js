...
class State {
    ...
    update(changes, reparse = true) {
        ...
        if (!reparse)
            return new State(doc, Tree.empty, fragments);
        let tree = parser.parse(doc, fragments);
        return new State(doc, tree, TreeFragment.addTree(tree, fragments));
    }
}
let _state1 = null, state1 = () => _state1 || (_state1 = State.start(doc1));
function overlap(a, b) {
    let inA = new Set(), shared = 0, sharingTo = 0;
    for (let cur = a.cursor(); cur.next();)
        if (cur.tree)
            inA.add(cur.tree);
    for (let cur = b.cursor(); cur.next();)
        if (cur.tree && inA.has(cur.tree) && cur.type.is("Block") && cur.from >= sharingTo) {
            shared += cur.to - cur.from;
            sharingTo = cur.to;
        }
    return Math.round(shared * 100 / b.length);
}
function testChange(change, reuse = 10) {
    let state = state1().update(change);
    compareTree(state.tree, parser.parse(state.doc));
    if (reuse)
        ist(overlap(state.tree, state1().tree), reuse, ">");
}
...
`;
        ...
        ist(tree.toString(), "Document(Paragraph(Emphasis(EmphasisMark,EmphasisMark)),BulletList(ListItem(ListMark,Paragraph(Emphasis(EmphasisMark,EmphasisMark)))))");
